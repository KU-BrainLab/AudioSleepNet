...
class AugmentMelSTFT(nn.Module):
    ...
    def forward(self, x):
        ...
        mel_basis, _ = torchaudio.compliance.kaldi.get_mel_banks(
            self.n_mels,  self.n_fft, self.sr, fmin, fmax, vtln_low=100.0, vtln_high=-500., vtln_warp_factor=1.0)
        mel_basis = torch.as_tensor(torch.nn.functional.pad(mel_basis, (0, 1), mode='constant', value=0),
                                    device=x.device)
        with torch.cuda.amp.autocast(enabled=False):
            mel_spec = torch.matmul(mel_basis, x)

        mel_spec = (mel_spec + 0.00001).log()

        if self.training:
            mel_spec = self.freqm(mel_spec)
            mel_spec = self.timem(mel_spec)

        mel_spec = (mel_spec + 4.5) / 5.  # fast normalization

        return mel_spec


def subject_group_cross_validation(base_path, n_splits=5):
    # Subject Variability
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(files)):
        temp[fold] = {
            ...
            'test_paths': list([os.path.join(base_path, f_name) for f_name in files[test_idx]])
        }
    return temp
